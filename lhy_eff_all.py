...
def E_field(mirrors,dis_matrix,Day,Hour,phi,H):
    ...
    for id in range(len(mirrors)):
        ...
        Ps_cos.append(efficiency["eta_cos"] * mirror[3] * mirror[4] * DNI_value)
        Ps_trunc.append(efficiency["eta_trunc"] * mirror[3] * mirror[4] * DNI_value)
        Ps_sb.append(efficiency["eta_sb"] * mirror[3] * mirror[4] * DNI_value)
        Ps.append(P)
    
    P_sum = sum(Ps)
    Area_sum = sum(Areas)
    P_per_area = P_sum/Area_sum
    P_cos_sum = sum(Ps_cos)
    P_trunc_sum = sum(Ps_trunc)
    P_sb_sum = sum(Ps_sb)
    P_without_all = Area_sum * DNI_value
    return Ps,P_sum,P_per_area,P_cos_sum,P_trunc_sum,P_sb_sum,P_without_all
